...
def initiate_part_payment(request: HttpRequest, room_id):
    ...
    # return redirect
    return render(request, 'payments/initiate_payment.html', {'room':room, 'is_part_payment':is_part_payment, 'user':request.user,})


def complete_part_payment(request: HttpRequest):
    try:
        tenant=Tenant.objects.get(user=request.user, made_part_payment=True)
        ...
        return render(request=request,template_name='payments/make_payment.html', 
                    context={'amount':payment.amount,'reference_id': payment.reference_id, 
                            ...
                            'user':request.user, 'is_completing_payment':True,
